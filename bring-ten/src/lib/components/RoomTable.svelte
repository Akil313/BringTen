...
<div class="flex h-full flex-col rounded-xl bg-gray-200">
	...
	{#if Object.values(rooms).length === 0}
		<div class="flex grow items-center justify-center">
			<span class="text-[2em]"> There are no rooms available</span>
		</div>
	{:else}
			...
			{#each Object.values(rooms) as r (r)}
				<div class="grid grid-cols-5 items-center rounded-md bg-white">
					...
					<button
						...
						class="joinGameBtn focus:shadow-outline w-full rounded {r.numPlayers < 4
